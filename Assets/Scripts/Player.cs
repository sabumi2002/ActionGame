...
public class Player : MonoBehaviour
{
    ...
    void Move()
    {
        ...
        if (isSwap || !isFireReady || isReload || isDead) //스왑하거나 공격할때 이동x
    ...
    void Dodge()
    {
        if (jDown && moveVec != Vector3.zero && !isJump && !isDodge && !isSwap && !isShop && !isDead)    //움직이면서 점프할때 회피모션
        {
